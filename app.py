...
# stm.title("Customer Churn Preiction")
# geography = stm.selectbox("Geography", ohe.categories_[0])
# gender = stm.selectbox("Gender", le.classes_)
# age = stm.slider("Age", 18, 92)
# balance = stm.number_input("Balance")
# credit_score = stm.number_input("Credit Score")
# estimated_salary = stm.number_input("Estimated Salary")
# tenure = stm.slider("Tenure", 0, 10)
# num_of_products = stm.slider("Number of Prodcuts", 1, 4)
# has_cr_card = stm.selectbox("Has Credit Card", [0,1])
# is_active_member = stm.selectbox("Is Active Memer", [0,1])
...
# df = pd.concat([input_data, geo_encoded_df], axis='columns')
...
# if pred_prob > 0.5:
#     stm.write("The customer is likely to churn!")
# else:
#     stm.write("The customer is unlikely to churn!")
